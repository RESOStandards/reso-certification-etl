const CURRENT_REFERENCE_METADATA_REPORT_JSON = require("./references/dd-1.7/metadata-report.json");
const { standardMetadata } = require("./references/dd-1.7/standard-metadata");
const {
  mergeLookupResourceMetadata,
} = require("./process-lookup-resource-metadata");
const {
  getFieldsCount,
  getResourcesCount,
  getLookupsCount,
...
const sampleMetadata = require("../metadata-report.json");
const lookupResourceMetadata = require("../lookup-resource-lookup-metadata.json");
...
const processMetadataReport = (
  metadataReportJson = sampleMetadata,
  lookupResourceLookupsJson = lookupResourceMetadata
) => {
  ...
  const hasLookupResourceMetadata = lookupResourceLookupsJson?.lookups?.length;
  ...
  if (hasLookupResourceMetadata) {
    const { fields: mergedFields = [], lookups: mergedLookups = [] } =
      mergeLookupResourceMetadata({
        metadataReportJson,
        lookupResourceLookupsJson,
      }) || {};
    transformedMetadataReportJson.fields.push(
      ...getTransformedFields(mergedFields)
    );
    transformedMetadataReportJson.lookups.push(
      ...getTransformedLookups({
        ...metadataReportJson,
        fields: mergedFields,
        lookups: [...lookups, ...mergedLookups],
      })
    );
  } else {
    transformedMetadataReportJson.fields.push(...getTransformedFields(fields));
    transformedMetadataReportJson.lookups.push(
      ...getTransformedLookups(metadataReportJson)
    );
  }
