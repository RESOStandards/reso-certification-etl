 ...
 */
const processMetadataReport = (metadataReportJson = {}) => {
  const { fields, lookups } = metadataReportJson;
...
const parseLookupName = (lookupName = "") => {
  ...
  return {
    lookupName:
      lookupName?.slice(
        lookupName?.lastIndexOf(FQDNSeparator) + 1,
        lookupName?.length
      ) || null,
    lookupNameFQDN:
...
const getTransformedLookups = (metadataReportJson = {}) => {
  ...
  //TODO: add support for Lookup Resource items
