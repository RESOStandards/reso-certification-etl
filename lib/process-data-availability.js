...
const { lookupMap } = require('./references/dd-1.7/lookup-map');
...
const createLookupValueCache = (lookupValues = []) =>
  lookupValues.reduce((cache, item) => {
    const { resourceName, fieldName, lookupValue } = item;
...
const processDataAvailability = async (availabilityReport) => {
  ...
  const { resources, fields, lookups, lookupValues } = availabilityReport;
  ...
  fields.forEach((field) => {
    ...
    const lookupsForField =
      lookupValueCache[resourceName] &&
      lookupValueCache[resourceName][field.fieldName];
      ...
      Object.values(lookupsForField).forEach((lookupValue) => {
        if (lookupValue && lookupValue?.lookupValue !== 'NULL_VALUE') {
          ...
          processedLookupValues.push({
            ...
            ...computeBooleanBins(
              computeBins(lookupAvailability, getBinsTemplate())
            ),
          });
        }
      });
