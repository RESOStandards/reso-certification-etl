...
const processDataAvailability = async (availabilityReport) => {
  ...
  // rollup expansions nested in resources
  const rolledUpResources = resources.flatMap((resource) => {
    const { expansions = [], ...rest } = resource;
    const parentResourceName = resource.resourceName;
    const expansionResources = expansions.map(expansion => {
      expansion.parentResourceName = parentResourceName;
      return expansion;
    });
    expansionResources.push(rest);
    return expansionResources;
  });

  transformed.resources = rolledUpResources;
