...
const parentPrefix = string => '__parent__' + string;
...
const processDataAvailability = async availabilityReport => {
  ...
  const nestedResourcesCount = {};
  resources.forEach(resource => (resourceCounts[resource.resourceName] = resource.numRecordsFetched));
  resources.forEach(resource => {
    if (resource.expansions) {
      resource.expansions.forEach(expansion => {
        if (!nestedResourcesCount[resource.resourceName]) {
          nestedResourcesCount[resource.resourceName] = {};
        }
        nestedResourcesCount[resource.resourceName][expansion.resourceName] = expansion.numRecordsFetched;
      });
    }
  });
  ...
  const processedFields = [],
    ...
    lookupValueCacheWithoutParentResourceName = createLookupValueCache(lookupValuesWithoutParentResourceName),
    lookupValueCacheWithParentResourceName = createLookupValueCache(lookupValuesWithParentResourceName);
  ...
  fields.forEach(field => {
    ...
    const count = parentResourceName ? nestedResourcesCount[parentResourceName]?.[resourceName] : resourceCounts[resourceName];
    ...
    updateAvailabilityCache({
      ...
      statType: 'total' // total, reso, idx, local
    });
    ...
    if (isResoField(resourceName, fieldName, standardFieldCache)) {
      ...
      updateAvailabilityCache({
        ...
        parentResourceName,
        type: 'fields', // fields or lookups
        statType: 'reso' // total, reso, idx, local
      });
      ...
      if (isIdxField(resourceName, fieldName, standardFieldCache)) {
        ...
        updateAvailabilityCache({
          ...
          parentResourceName,
          type: 'fields', // fields or lookups
          statType: 'idx' // total, reso, idx, local
        });
      ...
      updateAvailabilityCache({
        ...
        parentResourceName,
        type: 'fields', // fields or lookups
        statType: 'local' // total, reso, idx, local
      });
    ...
    const lookupsForField = (() => {
      if (parentResourceName) {
        return (
          ...
          lookupValueCacheWithParentResourceName[resourceName][fieldName][parentPrefix(parentResourceName)]
        );
      } else {
        return (
          lookupValueCacheWithoutParentResourceName[resourceName] && lookupValueCacheWithoutParentResourceName[resourceName][fieldName]
        );
      }
    })();
...
const updateResourceBinaryAvailability = ({ resourceName, type, statType, availability, resourcesBinary, parentResourceName } = {}) => {
  if (parentResourceName) {
    resourcesBinary[parentResourceName].expansions[resourceName][type][statType] = computeBins(
      availability,
      resourcesBinary[parentResourceName].expansions[resourceName][type][statType]
    );
  } else {
    resourcesBinary[resourceName][type][statType] = computeBins(availability, resourcesBinary[resourceName][type][statType]);
  }
};
...
const updateAvailabilityCache = ({
  ...
  if (parentResourceName) {
    availabilityAggregatesByResource[parentResourceName].expansions[resourceName][type][statType] += availability;
    availabilityAggregatesByResource[parentResourceName].expansions[resourceName][countType][statType]++;
  } else {
    availabilityAggregatesByResource[resourceName][type][statType] += availability;
    availabilityAggregatesByResource[resourceName][countType][statType]++;
  }
};
...
const updateAvailability = ({ acc, resourceName, expansionResourceName, type, data, availabilityAggregatesByResource }) => {
  const statTypes = ['total', 'reso', 'idx', 'local'];
  const countType = type === 'fields' ? 'fieldCounts' : 'lookupCounts';
  statTypes.forEach(statType => {
    if (expansionResourceName) {
      acc[resourceName].expansions[expansionResourceName][type][statType] =
        (1.0 * data[statType]) /
        (availabilityAggregatesByResource[resourceName]?.expansions[expansionResourceName]?.[countType]?.[statType] || 1);
    } else {
      acc[resourceName][type][statType] =
        (1.0 * data[statType]) / (availabilityAggregatesByResource[resourceName]?.[countType]?.[statType] || 1);
    }
  });
};
