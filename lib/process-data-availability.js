const { getLookupMap, getStandardMetadata, CATEGORIES, SUPPORTED_DD_VERSIONS, CURRENT_DD_VERSION } = require('./common');
const lookupMap = SUPPORTED_DD_VERSIONS.reduce((acc, version) => {
  acc[version] = getLookupMap(version);
  return acc;
}, {});
const standardMetadata = SUPPORTED_DD_VERSIONS.reduce((acc, version) => {
  acc[version] = getStandardMetadata(version);
  return acc;
}, {});
 ...
 * @param {Object} obj the name of the resource for the field.
 * @param {String} obj.resourceName the name of the resource for the field.
 * @param {String} obj.fieldName the name of the field.
 * @param {String} obj.lookupValue the name of the lookup to test.
 * @param {Object} obj.standardFieldCache a field cache created by createStandardFieldCache().
 * @param {String} obj.version DD version.
 * @returns the RESO lookup, if found, otherwise null.
 */
const findResoLookup = ({ resourceName, fieldName, lookupValue, standardFieldCache = {}, version }) => {
  if (resourceName && fieldName && standardFieldCache[resourceName] && standardFieldCache[resourceName][fieldName]) {
    ...
    if (field && field.simpleDataType?.includes('String List') && field.type?.includes('.')) {
      ...
      const lookup = lookupMap?.[version]?.[lookupName]?.find(x => x?.lookupValue === lookupValue || x?.lookupDisplayName === lookupValue);
...
const processDataAvailability = async (availabilityReport, useNewReportFormat = true) => {
  ...
  const standardFieldCache = createStandardFieldCache(standardMetadata?.[rest?.version || CURRENT_DD_VERSION]?.fields);
      ...
      Object.values(lookupsForField).forEach(lookupValue => {
        if (lookupValue && lookupValue?.lookupValue !== 'NULL_VALUE') {
          ...
          if (
            ...
            findResoLookup({
              resourceName: lookupValue.resourceName,
              fieldName: lookupValue.fieldName,
              lookupValue: lookupValue.lookupValue,
              standardFieldCache,
              version: rest?.version || CURRENT_DD_VERSION
            })
          ) {
            //update resource availability and counts
            updateAvailabilityCache({
