...
const getMetadata = (version = CURRENT_DD_VERSION) =>
  require(`./reference-metadata/dd-${version}.json`) || {};
...
const getReferenceLookupMap = async (version = CURRENT_DD_VERSION) =>
  (await getMetadata(version)).lookups.reduce(
    (acc, { lookupName, lookupValue, annotations }) => {
      const parsedLookupName = lookupName?.substring(
        lookupName?.lastIndexOf(".") + 1
      );
      const annotationMap = buildAnnotationMap(annotations);

      if (parsedLookupName && !acc[parsedLookupName])
        acc[parsedLookupName] = [];
